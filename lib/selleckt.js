    ...
    */
    ...
    _.extend(SingleSelleckt.prototype, {
        ...
        initialize: function(options){
            ...
            this.template = parseTemplate(settings.mainTemplate);
        ...
        bindEvents: function(){
            ...
            function highlightItem($item){
                ...
                $items.scrollTop($item.position().top - $item.height());
            }
            ...
            $sellecktEl.on('click', itemClass, function(e){
                e.stopPropagation();
            ...
            $sellecktEl.on('keydown', function(e){
                ...
                if(whichKey === KEY_CODES.DOWN){
                    ...
                    return highlightItem($currentHighlightItem.length ?
                                            $currentHighlightItem.nextAll('.'+self.itemClass+':visible').first()
                                            : $theItems.filter(':visible').eq(0));
                }
                ...
                if(whichKey === KEY_CODES.UP){
                    ...
                    if(!$currentHighlightItem.length){
                        return;
                    }

                    return highlightItem($currentHighlightItem.prevAll('.'+self.itemClass+':visible').first());
                }
        ...
        render: function(){
            var selectedItem = this.getSelection(),
                $sellecktEl = this.$sellecktEl = $(this.template({
    ...
    MultiSelleckt.prototype.initialize = function(options){
        ...
        this.selectionTemplate = parseTemplate(options.selectionTemplate);
