    ...
    */
    ...
    _.extend(SingleSelleckt.prototype, {
        ...
        _parseItemsFromOptions: function($selectEl){
            ...
            if ($selectEl.find('optgroup').length) {
                var ungrouped = _($selectEl.find('> option')).reduce(parseOptions, {
                    items: [],
                    selectedItems: []
                });
                var groupedItems = _($selectEl.find('optgroup')).reduce(function(memo, group){
                    var
                        $group = $(group),
                        groupLabel = group.label,
                        items = _($group.find('option')).reduce(function(groupMemo, option) {
                            var $option = $(option),
                                item = {
                                    value: $option.val(),
                                    label: $option.text(),
